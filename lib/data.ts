...
export async function notices(user: HubUser, includeArchived = false, query = "") {
  ...
  const result = await db().execute({ sql: `SELECT n.*,u.full_name as authorName FROM notices n JOIN users u ON u.id=n.author_id WHERE (?=1 OR (n.status!='archived' AND n.publish_at<=CURRENT_TIMESTAMP AND (n.expires_at IS NULL OR n.expires_at>CURRENT_TIMESTAMP))) AND (?='' OR lower(n.title||' '||n.body||' '||n.category||' '||u.full_name) LIKE '%'||lower(?)||'%') ORDER BY CASE n.priority WHEN 'critical' THEN 0 WHEN 'important' THEN 1 ELSE 2 END,n.publish_at DESC`, args: [includeArchived ? 1 : 0, query, query] });
...
export async function documents(user:HubUser,query=""){const result=await db().execute({sql:`SELECT d.*,dep.name as departmentName,u.full_name as updatedByName FROM documents d LEFT JOIN departments dep ON dep.id=d.department_id LEFT JOIN users u ON u.id=d.updated_by WHERE ?='' OR lower(d.title||' '||d.description||' '||d.category||' '||coalesce(dep.name,'')) LIKE '%'||lower(?)||'%' ORDER BY d.pinned DESC,d.updated_at DESC`,args:[query,query]});return visible(user,"document",rows<RecordRow>(result));}
...
export async function directory(query=""){return rows<RecordRow>(await db().execute({sql:`SELECT u.id,u.full_name as fullName,u.email,u.role,u.job_title as jobTitle,u.phone,d.name as departmentName FROM users u LEFT JOIN departments d ON d.id=u.department_id WHERE u.active=1 AND (?='' OR lower(u.full_name||' '||u.email||' '||u.job_title||' '||coalesce(d.name,'')) LIKE '%'||lower(?)||'%') ORDER BY u.full_name`,args:[query,query]}));}
