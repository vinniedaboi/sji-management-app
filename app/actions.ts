...
export async function acknowledge(formData:FormData){const user=await requireUser();const entityType=text(formData,"entityType"),entityId=text(formData,"entityId");await db().execute({sql:`INSERT INTO acknowledgements(id,entity_type,entity_id,user_id) VALUES (?,?,?,?) ON CONFLICT (entity_type,entity_id,user_id) DO NOTHING`,args:[uuid(),entityType,entityId,user.id]});revalidatePath("/");revalidatePath(`/${entityType==="notice"?"notices":"documents"}/${entityId}`);}
